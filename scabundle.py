...
def ona_settings_and_logs():
    ...
    if os.path.exists("/etc/ise_poller"):
        copytree("/etc/ise_poller", f'{bundledir}/etc/ise_poller/', ignore=ignore_patterns('*key*'), copy_function=copy)
    if os.path.exists("/tmp/ise_poller"):
        copytree("/tmp/ise_poller", f'{bundledir}/tmp/ise_poller/', ignore=ignore_patterns('*key*'), copy_function=copy)
